# XLog v1.1.1 - New Features Documentation

## Overview

Version 1.1.1 introduces four developer-focused features that enhance XLog's capabilities for production environments:
...
auto severity_filter = std::make_shared<xlog::RegexFilter>(
    "severity", 
    "(high|critical)"
);
...
composite->add_filter(std::make_shared<xlog::RegexFilter>("ERROR"));
composite->add_filter(std::make_shared<xlog::RegexFilter>("(password|token)", true));
...
// Set initial level
logger->set_level(xlog::LogLevel::Info);
...
if (xlog::HealthChecker::is_healthy(result)) {
    ...
    std::cout << "⚠️ Issues detected: " << result.message << "\n";
}
```
...
std::string handle_health_request(const HttpRequest& req) {
    ...
    if (logger_name.empty()) {
        // Check all loggers
        return xlog::HealthRegistry::instance().export_json();
    } else {
        // Check specific logger
        auto result = xlog::HealthRegistry::instance().check_logger(logger_name);
...
xlog::HealthChecker checker(config);
```
...
std::string prom_metrics = metrics.export_prometheus("xlog");

// Combine with health status
auto health = checker.check_metrics(metrics);
if (health.status != xlog::HealthStatus::Healthy) {
    prom_metrics += "xlog_health_status{status=\"degraded\"} 1\n";
}
```
...
auto sink = std::make_shared<xlog::CompressedFileSink>(..., options);
...
**Decision Logic:**
- **Slow + Good Ratio** → Decrease level (faster compression)
- **Fast + Poor Ratio** → Increase level (better compression)
- **Excellent Ratio** → Try decreasing level (may still be good)
...
auto filter = std::make_shared<xlog::RegexFilter>("(password|token)", true);
logger->add_filter(filter);

// 2. Use atomic log level changes
logger->set_level_dynamic(xlog::LogLevel::Debug);  // Instead of set_level()

// 3. Add health monitoring
xlog::HealthRegistry::instance().register_logger("my_logger", logger);
...
- **Issues**: [GitHub Issues](https://github.com/hent83722/xlog/issues)
