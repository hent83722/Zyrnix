# XLog v1.1.0 - New Features

## Overview

Version 1.1.0 introduces four major enterprise-grade features that enhance XLog's capabilities for production environments:
...
```cpp
xlog::AzureMonitorSink::Config config;
config.instrumentation_key = "your-key-here";
config.cloud_role_name = "my-service";
config.batch_size = 100;

auto sink = std::make_shared<xlog::AzureMonitorSink>(config);
auto logger = std::make_shared<xlog::Logger>("app");
logger->add_sink(sink);
...
```
xlog_messages_logged_total          # Total messages logged
xlog_messages_dropped_total         # Total messages dropped
xlog_messages_filtered_total        # Total messages filtered
xlog_messages_per_second           # Current logging rate
xlog_log_latency_us_avg            # Average log latency
xlog_log_latency_us_max            # Maximum log latency
xlog_queue_depth                   # Current queue depth
xlog_queue_depth_max               # Maximum queue depth
xlog_errors_total                  # Total errors
xlog_sink_writes_total{sink=""}    # Writes per sink
xlog_sink_bytes_written_total{sink=""} # Bytes per sink
```
...
```bash
cd examples
g++ -std=c++17 rate_limiting_example.cpp -lxlog -o rate_limiting_example
...
```cpp
// Before (v1.0.4)
auto sink = std::make_shared<xlog::RotatingFileSink>("app.log", max_size, max_files);

// After (v1.1.0) - with compression
xlog::CompressionOptions opts;
opts.type = xlog::CompressionType::Gzip;
auto sink = std::make_shared<xlog::CompressedFileSink>("app.log", max_size, max_files, opts);
```
